...
<!-- end Search Doctor -->
    ...
    <input type="search" placeholder="Please Search ambulance  Name Only" name="search" class="searchbar-input" onkeyup="buttonUp();" required> 
...
<div class="row">
  ...
  <div class="carousel-inner">
        <?php  
        include "admin/admin/include/conn.php";
        $sql="select file_name from tbl_slider where catid=6";
        ...
        include"admin/admin/include/conn.php";
				$sr=1;
				   if(isset($_POST['search'])){
                     $sreach= $_POST['search'];
				      $sql="SELECT * FROM  `tbl_ambulance` WHERE name LIKE '%$sreach%';";  
				    
				   }else if(isset($_POST['search2'])){
                     $cat= $_POST['cat'];
                     $state= $_POST['state'];
                     $dis= $_POST['dis'];
				       $sql="SELECT * FROM  `tbl_ambulance` WHERE dcategory='$cat' and state=$state and dis= $dis";  
...
<div class="card">
  ...
  <img class="card-img-top" src="admin/admin/ambulance-image/<?=$list['image'];?>" style="height: 200px;width:200px" class="rounded mx-auto d-block"  alt="Card image cap">
  </div>
  <div class="card-body">
   
    <p class="card-text font-weight-bold">Driver Name - <?=$list['name'];?></p>
  </div>
  <ul class="list-group list-group-flush">
    
    <li class="list-group-item">Driver Mobile <span class="pull-right"><i class="fa fa-mobile" aria-hidden="true"></i> <?=$list['mobile'];?> </span></li>
